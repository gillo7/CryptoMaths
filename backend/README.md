...
| [tls/](tls/) | `/api/tls/scan` | 8102 | Powers the TLS chapter's live demo - echoes back the browser's own just-negotiated TLS protocol/cipher (via nginx's `$ssl_protocol`/`$ssl_cipher` forwarded as headers) alongside a live scan of this server's own listener across six real handshake attempts (TLS 1.0/1.1 rejected, 1.2/1.3 accepted, one legacy and one modern TLS 1.2 cipher to show the server's cipher policy directly) |
